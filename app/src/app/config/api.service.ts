...
export class ApiService {
     ...
     * @param {Model} [body] - The body model of the request (ie: Event, Blog...).
     ...
     */
    public apiBodyFetch<Model>(endpoint: string, method: HttpBodyMethod, onSuccess: Function, body: Model, headers?: HttpHeaders) {
        let req: Observable<any> = this.http[method](`${this.baseUrl}${endpoint}`, body ?? {}, { headers: headers }).pipe(catchError(this.handleError));
     ...
     */
    public apiFetch(endpoint: string, method: HttpMethod, onSuccess: Function, headers?: HttpHeaders) {
        let req: Observable<any> = this.http[method](`${this.baseUrl}${endpoint}`, { headers: headers }).pipe(catchError(this.handleError));
        return req.subscribe((res) => onSuccess(res));
    }
